...
public class ID3V2Frame {
	...
	public ID3V2Frame(String _frameID, int _sizeLength, long _size, INumericPrimitive _flags) {
		...
		if (!isLastTag()) {
			...
			if (myFrameSizeUnsynchronized) {
				...
				if (mySizeLength == 4) {
					...
					mySize = ID3V2Tag.ss24_to_cpu(sizeBuf);
				}
			} else {
				if (mySizeLength == 4) {
					mySize = ID3V2Tag.p32_to_cpu(sizeBuf);
				} else {
					mySize = ID3V2Tag.p24_to_cpu(sizeBuf);
				}
			}
			myFlags.read(_is);
		}
