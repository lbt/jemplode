...
/**
 * Provides an implementation of SSDP device discovery.
 */
public class SSDPDiscoverer extends AbstractDiscoverer {
	...
	/**
	 * Constructs an SSDPDiscoverer.
	 * 
	 * @param _uris SSDP URIs to lookup 
	 */
	...
	protected void startDiscovery0() {
		...
		try {
			...
			//			receiveSocket.setReuseAddress(false);
			...
			InetAddress[] addresses = new InetAddress[localAddresses.length + 1];
			System.arraycopy(localAddresses, 0, addresses, 1, localAddresses.length);
			
			mySendSockets = new DatagramSocket[addresses.length];
			for (int i = 0; i < addresses.length; i ++) {
				final DatagramSocket sendSocket = new DatagramSocket(0, addresses[i]);
				Debug.println(Debug.INFORMATIVE, "SSDPDiscoverer.startDiscovery0: " + addresses[i]);
					...
					public void run() {
						while (isRunning()) {
							try {
								...
								DatagramPacket receiveDp = new DatagramPacket(responseBytes, 0, responseBytes.length, group, port);
								sendSocket.receive(receiveDp);
	
								InetAddress responseHost = receiveDp.getAddress();
								String responseData = new String(receiveDp.getData(), 0, receiveDp.getLength());
								...
								SSDPDevice device = new SSDPDevice(responseHost, responseProps);
			...
			for (int i = 0; i < myURIs.length; i ++) {
				...
				DatagramPacket sendDp = new DatagramPacket(requestBytes, 0, requestBytes.length, group, port);
				for (int socketNum = 0; socketNum < mySendSockets.length; socketNum ++) {
