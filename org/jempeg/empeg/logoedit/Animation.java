...
public class Animation {
  ...
  public void save(LittleEndianOutputStream _os) throws IOException {
    ...
    for (int i = 0; i < (maxAni - 1); i ++) {
      ...
      _os.writeUnsigned32(offset);
    }
    _os.writeUnsigned32(0);

    // write frames
    int maxFrame = getFrameCount();
    for (int frame = 0; frame < maxFrame; frame ++) {
      Image frameImage = getFrameAt(frame);
      byte[] frameBytes = LogoFormatUtils.toLogoFormat(frameImage, myWidth, myHeight, LogoFormatUtils.DEFAULT_CUTOFFS);
      MemorySeekableInputStream is = new MemorySeekableInputStream(frameBytes);
      for (int i = 0; i < 1024; i ++) {
        int p1 = is.read();
        int p2 = is.read();
        int animValue = ((p1 & 0x30) << 2) | ((p1 & 0x03) << 4) | ((p2 & 0x30) >> 2) | (p2 & 0x03);
  ...
  public void load(LittleEndianInputStream _is, long _totalLength) throws IOException {
    myFrames.removeAllElements();
    mySequence.removeAllElements();
    ...
    int maxFrame;
    if (_totalLength >= 0) {
      maxFrame = (int)(_totalLength - ((mySequence.size() + 1) * 4)) / 1024;
    } else {
      maxFrame = -1;
    }
    ...
    for (int frame = 0; (maxFrame == -1) ? !done : (frame < maxFrame); frame ++) {
      int byteNum = 0;
      ...
      if (!done) {
        ...
        Image frameImage = LogoFormatUtils.fromLogoFormat(myComponent, frameBytes, myWidth, myHeight, LogoFormatUtils.DEFAULT_GRAY_VALUES);
    ...
    for (int i = 0; i < (maxAni - 1); i ++) {
      ...
      int frame = (offset - (maxAni * 4)) / 1024;
      Image frameImage = (Image)myFrames.elementAt(frame);
      mySequence.setElementAt(frameImage, i);
    }
  }

  public int[] getPixels(Image _img) {
    int[] tempPixels = new int[_img.getWidth(myComponent) * _img.getHeight(myComponent)];
    PixelGrabber pg = new PixelGrabber(_img, 0, 0, _img.getWidth(myComponent), _img.getHeight(myComponent), tempPixels, 0, _img.getWidth(myComponent));
    try {
      pg.grabPixels();
    }
  ...
  public void loadAnimatedGif(InputStream _is) throws IOException {
    ...
    for (int i = 0; i < gifFrameCount; i ++) {
      ...
      for (int j = 0; !foundMatch && j < animFrameCount; j ++) {
        ...
        if (pixelsMatch) {
          gifFrame = animFrame;
          Debug.println(Debug.INFORMATIVE, "Sequence " + i + " matches frame " + j);
          foundMatch = true;
        }
      }
...
//    Animation animation = new Animation(this, LogoFormatUtils.DEFAULT_WIDTH, LogoFormatUtils.DEFAULT_HEIGHT);
//    String src = getParameter("src");
//    URL codeBase = getCodeBase();
//    URL animationUrl = new URL(codeBase, src);
//    InputStream is = animationUrl.openStream();
//    EmpegInputStream eis = new EmpegInputStream(is);
