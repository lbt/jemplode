...
public class SearchPanel extends JComponent {
	...
	protected void showSearchResults(FIDPlaylist _playlist) {
		if (_playlist != null) {
			FIDPlaylistTableModel searchResultsTableModel = new FIDPlaylistTableModel(_playlist);
			SortedPlaylistTableModel sortedSearchResultsTableModel = new SortedPlaylistTableModel(searchResultsTableModel);
	...
	protected class TextOption implements ActionListener {
		...
		public IPredicate getPredicate() {
			...
			LikePredicate pred = new LikePredicate(new TagValue(myTagName), new LiteralValue(value));
			return pred;
		}
