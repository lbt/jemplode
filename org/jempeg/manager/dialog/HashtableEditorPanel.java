/* HashtableEditorPanel - Decompiled by JODE
...
public class HashtableEditorPanel extends JPanel
{
	...
	DialogUtils.addRow(new JLabel("New Name"), myNameTextField,
			   gridBagLayout, textFieldPanel);
	DialogUtils.addRow(new JLabel("New Value"), myValueTextField,
			   gridBagLayout, textFieldPanel);
	...
	editorPanel.add(textFieldPanel, "Center");
	editorPanel.add(buttonPanel, "South");
	JScrollPane tableScrollPane = new JScrollPane(myTable);
	this.add(tableScrollPane, "Center");
	this.add(editorPanel, "South");
	validateAdd();
	validateDelete();
    }
	...
	boolean valid = (name != null && value != null && name.length() > 0
			 && value.length() > 0);
	return valid;
    }
