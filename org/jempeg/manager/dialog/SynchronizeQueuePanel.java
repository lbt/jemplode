/* SynchronizeQueuePanel - Decompiled by JODE
...
public class SynchronizeQueuePanel extends JPanel implements IContextListener
{
    ...
    public SynchronizeQueuePanel(ApplicationContext _context,
				 boolean _dualProgressBars) {
	myContext = _context;
	mySynchronizeQueueTable = new JTable();
	JPopupMenu synchronizeQueueMenu = new JPopupMenu();
	final JMenuItem cancelDatabaseChange
	    = UIUtils.createMenuItem("cancel");
	cancelDatabaseChange.addActionListener
	    (new DatabaseChangeCancelAction(myContext,
					    mySynchronizeQueueTable));
	synchronizeQueueMenu.add(cancelDatabaseChange);
	mySynchronizeQueueTable.getSelectionModel()
	    .addListSelectionListener(new ListSelectionListener() {
	    public void valueChanged(ListSelectionEvent _event) {
		if (!_event.getValueIsAdjusting())
		    cancelDatabaseChange.setEnabled(mySynchronizeQueueTable
							.getSelectedRowCount()
						    > 0);
	    }
	});
	mySynchronizeQueueTable
	    .addMouseListener(new DefaultPopupListener(synchronizeQueueMenu));
	...
	syncControlPanel.add(myButtonPanel, "North");
	if (_dualProgressBars) {
	    mySynchronizeProgressBar = new ProgressPanel(true, false);
	    mySynchronizeProgressBar
		.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
	    _context.setSynchronizeProgressListener((IProgressListener)
						    mySynchronizeProgressBar);
	} else
	    mySynchronizeProgressBar = new JProgressBar();
	syncControlPanel.add(mySynchronizeProgressBar, "South");
	add(syncControlPanel, "South");
	...
	if (_newPlayerDatabase != null) {
	    ...
	    SynchronizeQueueTableModel syncTableModel
		= new SynchronizeQueueTableModel(_newPlayerDatabase
						     .getSynchronizeQueue(),
						 synchronizeClient);
		...
		((JProgressBar) mySynchronizeProgressBar)
		    .setModel(syncTableModel.getCompletionModel());
	}
    }
	...
	if (tableModel instanceof SynchronizeQueueTableModel) {
	    SynchronizeQueueTableModel syncQueueTableModel
		= (SynchronizeQueueTableModel) tableModel;
