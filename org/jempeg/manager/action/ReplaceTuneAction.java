			...
			if (selection.getSize() != 1) {
				...
				if (retVal == JFileChooser.APPROVE_OPTION) {
					...
					if (selectedFiles.length > 0) {
						LocalImportFile selectedFile = new LocalImportFile(selectedFiles[0]);
						PlayerDatabase playerDatabase = myContext.getPlayerDatabase();
						IFIDNode existingNode = FIDLocalFile.getExistingInstance(playerDatabase, selectedFile);
						if (existingNode == null) {
							IFIDNode createdNode = FIDLocalFile.createInstance(playerDatabase, selectedFile, true, false);
              Properties tagProperties = createdNode.getTags().toProperties();
              ...
              while (tagNamesEnum.hasMoreElements()) {
                ...
                if (!DatabaseTags.FID_TAG.equals(tagName) && !DatabaseTags.CTIME_TAG.equals(tagName)) {
                  String tagValue = (String)tagProperties.get(tagName);
                  nodeToReplace.getTags().setValue(tagName, tagValue);
                }
              }
							playerDatabase.getSynchronizeQueue().enqueue(new NodeReplacedDatabaseChange(nodeToReplace, selectedFile));
						}
						else {
