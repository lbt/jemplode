/* DatabaseChangeCancelAction - Decompiled by JODE
...
public class DatabaseChangeCancelAction extends AbstractAction
{
    ...
    public DatabaseChangeCancelAction(ApplicationContext _context,
				      JTable _synchronizeQueueTable) {
	myContext = _context;
	...
	SynchronizeQueue queue
	    = myContext.getPlayerDatabase().getSynchronizeQueue();
	Object object;
	MONITORENTER (object = queue.getLockObject());
	MISSING MONITORENTER
	synchronized (object) {
	    SynchronizeQueueTableModel tableModel
		= ((SynchronizeQueueTableModel)
		   mySynchronizeQueueTable.getModel());
	    int[] selectedRows = mySynchronizeQueueTable.getSelectedRows();
	    IDatabaseChange[] databaseChanges
		= new IDatabaseChange[selectedRows.length];
	    for (int i = 0; i < selectedRows.length; i++)
		databaseChanges[i]
		    = tableModel.getDatabaseChangeAt(selectedRows[i]);
	    for (int i = 0; i < databaseChanges.length; i++) {
		if (databaseChanges[i] != null
		    && databaseChanges[i] instanceof ICancelableDatabaseChange)
