...
public abstract class AbstractPlaylistContainerModifier extends AbstractContainerModifier {
  ...
  protected int copyTo(ApplicationContext _sourceContext, FIDPlaylist _targetPlaylist, Object _sourceObj, boolean _deepCopy, IProgressListener _progressListener, CollationKeyCache _cache) {
    PlayerDatabase localDatabase = _targetPlaylist.getDB();
    int copiedToIndex = -1;
    if (_sourceObj instanceof IContainer) {
      if (_sourceObj instanceof IFIDPlaylistWrapper && !_deepCopy && ((IFIDPlaylistWrapper) _sourceObj).getPlaylist().getDB() == localDatabase) {
        FIDPlaylist playlist = ((IFIDPlaylistWrapper) _sourceObj).getPlaylist();
        copiedToIndex = _targetPlaylist.addNode(playlist, true, _cache);
      }
      else {
        ...
        if (localDatabase.isNestedPlaylistAllowed()) {
          copiedToIndex = _targetPlaylist.getPlaylistIndex(name, false, true, _cache);
          newTargetPlaylist = _targetPlaylist.getPlaylistAt(copiedToIndex);
        ...
        for (int i = 0; (_progressListener == null || !_progressListener.isStopRequested()) && i < size; i ++ ) {
          childCopiedToIndex = copyTo(_sourceContext, newTargetPlaylist, container.getValueAt(i), _deepCopy, _progressListener, newCache);
        }
      ...
      if (node.getDB() != localDatabase) {
        ...
        if (node instanceof FIDLocalFile) {
          ...
          importFile = RemoteImportFile.createInstance(node, _sourceContext.getSynchronizeClient().getProtocolClient(new SilentProgressListener()));
        }
        ContainerModifierFactory.getInstance(_targetPlaylist).importFiles(new IImportFile[] {
          importFile
        }, null, _progressListener, true);
      }
      else {
