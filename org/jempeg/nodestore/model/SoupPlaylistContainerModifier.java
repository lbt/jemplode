...
/**
 * SoupPlaylistContainerModifier is a ContainerModifier that is implemented on
 * top of a soup playlist.  This is slightly different than
 * FIDPlaylistContainerModifier because you can't do things like delete from a
 * soup.
 * 
 * @author Mike Schrag
 */
public class SoupPlaylistContainerModifier extends AbstractPlaylistContainerModifier {
  ...
  public void delete(ContainerSelection _selection, IConfirmationListener _confirmationListener) {
    ...
    if (_confirmationListener == null) {
      ...
      String message = ResourceBundleUtils.getUIString("deleteConfirmation.soupMessage", new Object[] {
          new Integer(size),
          title
      });
    ...
    if (confirmation == IConfirmationListener.YES) {
      ...
      for (int i = 0; i < size; i ++ ) {
        ContainerUtils.traverse(_selection.getValueAt(i), deleteVec, new AllowAllFilter(), new NoNonSoupPlaylistsTraversalFilter());
      }
  ...
  protected IContainer importContainer(IContainer _targetContainer, IImportFolder _sourceFolder, FIDChangeSet _changeSet, IProgressListener _progressListener) {
    ...
    if (_sourceFolder instanceof M3UImportFile) {
      ...
      int newPlaylistIndex = rootPlaylist.getPlaylistIndex(_sourceFolder.getName(), IFIDNode.TYPE_PLAYLIST, IFIDNode.TYPE_TUNE, null, false, true, CollationKeyCache.createDefaultCache());
