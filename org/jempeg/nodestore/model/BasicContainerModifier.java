...
/**
 * BasicContainerModifier is an IContainerModifier for a BasicContainerTreeNode.
 * 
 * @author Mike Schrag
 */
  ...
  public void delete(ContainerSelection _selection, IConfirmationListener _confirmationListener) {
    if (_selection.getSize() == 1) {
      ...
      if (obj instanceof FIDPlaylistTreeNode) {
        ...
        int confirmation = _confirmationListener.confirm("Are you sure want to remove the " + playlist.getTitle() + " soup?", playlist);
        if (confirmation == IConfirmationListener.YES) {
          int index = _selection.getIndexAt(0);
          BasicContainerTreeNode containerTreeNode = (BasicContainerTreeNode) _selection.getContainer();
          containerTreeNode.remove(index);
          if (playlist.getReferenceCount() == 0) {
            playlist.delete();
          }
          DefaultTreeModel treeModel = playlistTreeNode.getTreeModel();
          treeModel.nodesWereRemoved(containerTreeNode, new int[] {
            index
          }, new Object[] {
            playlistTreeNode
          });
