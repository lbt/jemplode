...
/**
 * FIDPlaylistContainerModifier is a modifier for FIDPlaylists and
...
public class FIDPlaylistContainerModifier extends AbstractPlaylistContainerModifier {
  /**
   * Constructor for FIDPlaylistNodeModifier.
   */
  ...
  public void delete(ContainerSelection _selection, IConfirmationListener _confirmationListener) {
    ...
    if (_confirmationListener == null) {
      ...
      String message = ResourceBundleUtils.getUIString("deleteConfirmation.playlistMessage", new Object[] {
          new Integer(size),
          title
      });
      String checkboxMessage = ResourceBundleUtils.getUIString("deleteConfirmation.playlistCheckboxMessage");

      confirmation = _confirmationListener.confirm(message, checkboxMessage, null);
    }
    ...
    else if (confirmation == IConfirmationListener.YES_CHECKED) {
      Vector deleteVec = new Vector();
      for (int i = 0; i < size; i ++ ) {
        ContainerUtils.traverse(_selection.getValueAt(i), deleteVec, new AllowAllFilter(), new AllowAllTraversalFilter());
      }
  ...
  protected boolean importFile(IContainer _targetContainer, IFIDNode _node, FIDChangeSet _changeSet, CollationKeyCache _cache) {
    ...
    if (!alreadyExists) {
      Debug.println(Debug.VERBOSE, "Adding node... " + _node);
      targetPlaylist.addNode(_node, true, _cache);
    }
    return !alreadyExists;
  }

  protected IContainer importContainer(IContainer _targetContainer, IImportFolder _sourceFile, FIDChangeSet _changeSet, IProgressListener _progressListener) {
    FIDPlaylist targetPlaylist = (FIDPlaylist) _targetContainer;
    FIDPlaylist parentPlaylist;
    if (getPlayerDatabase().isNestedPlaylistAllowed()) {
      System.out.println("FIDPlaylistContainerModifier.importContainer: sourceFile = " + _sourceFile);
      String sourceFileName = _sourceFile.getName();
      System.out.println("FIDPlaylistContainerModifier.importContainer: targetContainer = " + _targetContainer);
      int childPlaylistIndex = targetPlaylist.getPlaylistIndex(sourceFileName, false, true, CollationKeyCache.createDefaultCache());
      System.out.println("FIDPlaylistContainerModifier.importContainer: childPlaylistIndex of " + sourceFileName + " in " + targetPlaylist + " = " + childPlaylistIndex);
      parentPlaylist = targetPlaylist.getPlaylistAt(childPlaylistIndex);
      System.out.println("FIDPlaylistContainerModifier.importContainer: parentPlaylist = " + parentPlaylist);
  ...
  public int[] copyTo(ContainerSelection _sourceSelection, IConfirmationListener _confirmationListener, boolean _deepCopy, IProgressListener _progressListener) {
    ...
    for (int i = 0; (_progressListener == null || !_progressListener.isStopRequested()) && i < selectionSize; i ++ ) {
      int index = copyTo(_sourceSelection.getContext(), targetPlaylist, selectedValues[i], _deepCopy, _progressListener, cache);
      if (index != -1) {
  ...
  public int[] moveTo(ContainerSelection _sourceSelection, IConfirmationListener _confirmationListener, IProgressListener _progressListener) {
    int[] indexes = copyTo(_sourceSelection, _confirmationListener, false, _progressListener);
    IContainerModifier nodeModifier = ContainerModifierFactory.getInstance(_sourceSelection);
