/* AbstractFIDPlaylistModel - Decompiled by JODE
...
public abstract class AbstractFIDPlaylistModel
    implements IContainer, IPlaylistListener, IFIDPlaylistWrapper
{
    private int myPlaylistIndex;
    private FIDPlaylist myPlaylist;
    private Vector myChildren;
    
    public AbstractFIDPlaylistModel(FIDPlaylist _playlist,
				    int _playlistIndex) {
	...
	MONITORENTER (fidplaylist = myPlaylist);
	MISSING MONITORENTER
	synchronized (fidplaylist) {
	    int size = myPlaylist.size();
	    for (int i = 0; i < size; i++) {
		FIDPlaylist playlist = myPlaylist.getPlaylistAt(i);
		if (playlist != null) {
		    AbstractFIDPlaylistModel childPlaylistModel
			= createChildModel(playlist, i);
	...
	if (myChildren != null) {
	    ...
	    for (int i = 0; i < size; i++) {
		AbstractFIDPlaylistModel childNode
		    = (AbstractFIDPlaylistModel) myChildren.elementAt(i);
    ...
    public synchronized void playlistNodeInserted
	(FIDPlaylist _parentPlaylist, IFIDNode _childNode, int _index) {
	if (myChildren != null && _parentPlaylist == myPlaylist
	    && _childNode instanceof FIDPlaylist) {
	    ...
	    if (index == -1)
		index = size;
	    AbstractFIDPlaylistModel newChildModel
		= createChildModel(childPlaylist, _index);
	    ...
	    notifyChildrenWereInserted(new int[] { index });
	}
    }
    
    public synchronized void playlistNodeRemoved
	(FIDPlaylist _parentPlaylist, IFIDNode _childNode, int _index) {
	if (myChildren != null && _parentPlaylist == myPlaylist) {
		...
		AbstractFIDPlaylistModel childPlaylistModel
		    = (AbstractFIDPlaylistModel) myChildren.elementAt(index);
		...
		notifyChildrenWereRemoved(new int[] { index },
					  new Object[] { childPlaylistModel });
	    } else {
		int size = myChildren.size();
		for (int i = 0; i < size; i++) {
		    AbstractFIDPlaylistModel childModel
			= (AbstractFIDPlaylistModel) myChildren.elementAt(i);
	...
	ensureChildrenLoaded();
	AbstractFIDPlaylistModel childPlaylistModel
	    = (AbstractFIDPlaylistModel) myChildren.elementAt(_index);
    ...
    protected abstract void notifyChildrenWereInserted(int[] is);
    
    protected abstract void notifyChildrenWereRemoved(int[] is,
						      Object[] objects);
    
    protected abstract AbstractFIDPlaylistModel createChildModel
	(FIDPlaylist fidplaylist, int i);
}
