...
public abstract class AbstractTagPlaylistSoupLayer implements IPlaylistSoupLayer {
  ...
  public AbstractTagPlaylistSoupLayer(String _tagName, int _containedType, String _sortTag) {
    this(NodeTag.getNodeTag(_tagName), _containedType, NodeTag.getNodeTag(_sortTag));
  }

  public AbstractTagPlaylistSoupLayer(NodeTag _tag, int _containedType, NodeTag _sortTag) {
    myTag = _tag;
  ...
  public FIDPlaylist[] getPlaylists(FIDPlaylist _soupPlaylist, IFIDNode _node, boolean _createIfMissing) {
    ...
    int playlistIndex = _soupPlaylist.getPlaylistIndex(playlistName, getType(), myContainedType, _node, false, _createIfMissing, mySortCache);
    FIDPlaylist[] playlists;
    if (playlistIndex < 0) {
      playlists = new FIDPlaylist[0];
    }
    else {
      FIDPlaylist newPlaylist = _soupPlaylist.getPlaylistAt(playlistIndex);
      playlists = new FIDPlaylist[] {
        newPlaylist
      };
      //newPlaylist.setSoup(true);
    }
  ...
  public boolean qualifies(IFIDNode _node) {
    boolean qualifies = _node.isIdentified() && !_node.isTransient() && !_node.isMarkedForDeletion() && !(_node instanceof FIDPlaylist);
    return qualifies;
  }
  ...
  protected abstract String getPlaylistName(String _value);
