...
public class SoupUpdater implements IDatabaseListener, INodeTagListener {
  ...
  public SoupUpdater(FIDPlaylist _soupPlaylist, ISoupLayer[] _soupLayers) {
    ...
    myTopLevelPlaylist = new FIDPlaylist[] {
      _soupPlaylist
    };
  ...
  public void initialize() {
    System.out.println("SoupUpdater.initialize: Initializing " + mySoupLayers[0] + " ...");

    Debug.println(Debug.INFORMATIVE, "SoupUpdater.initialize: initialize " + mySoupLayers[0]);
    FIDPlaylist topLevelPlaylist = myTopLevelPlaylist[0];
    if (!topLevelPlaylist.isTransient()) {
      // check for out-dated entries?
    }
  ...
  public void beforeNodeTagModified(IFIDNode _node, String _tag, String _oldValue, String _newValue) {
    if (_tag != DatabaseTags.TYPE_TAG && _tag != DatabaseTags.ICON_TYPE_TAG && isDependentOn(_tag) && !_node.isTransient()) {
      List possiblyEmptyPlaylistsList = new ArrayList(5);
      removeNode(myTopLevelPlaylist, _node, 0, possiblyEmptyPlaylistsList);
      myPossiblyEmptyPlaylistStack.push(possiblyEmptyPlaylistsList);
    }
  ...
  public void afterNodeTagModified(IFIDNode _node, String _tag, String _oldValue, String _newValue) {
    if (!_node.isTransient() && _tag != DatabaseTags.TYPE_TAG && _tag != DatabaseTags.ICON_TYPE_TAG && isDependentOn(_tag)) {
      addNodeIfQualified(myTopLevelPlaylist, _node);
      popAndRemovePlaylistsIfEmpty();
    }
  }
  ...
  private boolean isDependentOn(String _tagName) {
    ...
    Boolean isDependentOnBoolean = (Boolean) myDependentOnMap.get(_tagName);
  ...
  private boolean addNodeIfQualified(FIDPlaylist[] _playlists, IFIDNode _node) {
    boolean qualifies = true;
    for (int layerNum = 0; qualifies && layerNum < mySoupLayers.length; layerNum ++ ) {
  ...
  private void addNode(FIDPlaylist[] _playlists, IFIDNode _node, int _layerNum) {
    if (_layerNum == mySoupLayers.length) {
      ISoupLayer lastSoupLayer = mySoupLayers[_layerNum - 1];
      for (int playlistNum = 0; playlistNum < _playlists.length; playlistNum ++ ) {
        FIDPlaylist playlist = _playlists[playlistNum];
        playlist.setSortTag(lastSoupLayer.getSortTag(), false);
        int index = playlist.getIndexOf(_node, lastSoupLayer.getSortCache());
        if (index < 0) {
      ...
      if (soupLayer instanceof IPlaylistSoupLayer) {
        for (int playlistNum = 0; playlistNum < _playlists.length; playlistNum ++ ) {
          ...
          IPlaylistSoupLayer playlistSoupLayer = (IPlaylistSoupLayer) soupLayer;
          FIDPlaylist[] childPlaylists = playlistSoupLayer.getPlaylists(playlist, _node, true);
  ...
  private void removeNode(FIDPlaylist[] _playlists, IFIDNode _node, int _layerNum, List _possiblyEmptyPlaylistList) {
    if (_layerNum == mySoupLayers.length) {
      for (int playlistNum = 0; playlistNum < _playlists.length; playlistNum ++ ) {
      ...
      if (soupLayer.qualifies(_node)) {
        if (soupLayer instanceof IPlaylistSoupLayer) {
          ...
          for (int playlistNum = 0; playlistNum < _playlists.length; playlistNum ++ ) {
            FIDPlaylist playlist = _playlists[playlistNum];
            FIDPlaylist[] childPlaylists = playlistSoupLayer.getPlaylists(playlist, _node, false);
            removeNode(childPlaylists, _node, _layerNum + 1, _possiblyEmptyPlaylistList);
            _possiblyEmptyPlaylistList.add(playlist);
          }
        }
        else {
          removeNode(_playlists, _node, _layerNum + 1, _possiblyEmptyPlaylistList);
        }
      }
    }
  }
  ...
  private void popAndRemovePlaylistsIfEmpty() {
    List possiblyEmptyPlaylists = (List) myPossiblyEmptyPlaylistStack.pop();
