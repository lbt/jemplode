...
public class SearchSoupLayer implements ISoupLayer {
  ...
  public SearchSoupLayer(IPredicate _predicate) {
    this(_predicate, SoupLayerFactory.DEFAULT_CONTAINED_TYPE, NodeTag.TITLE_TAG);
  }

  public SearchSoupLayer(String _predicateStr, int _containedType, String _sortTag) throws ParseException {
    this(new PredicateParser().parse(_predicateStr), _containedType, NodeTag.getNodeTag(_sortTag));
  }

  public SearchSoupLayer(IPredicate _predicate, int _containedType, NodeTag _sortTag) {
  ...
  public boolean isDependentOn(NodeTag _tag) {
    return myPredicate.isDependentOn(_tag) || mySortTag.equals(_tag);
  }
  ...
  public boolean qualifies(IFIDNode _node) {
    boolean qualifies = _node.isIdentified() && !_node.isTransient() && !_node.isMarkedForDeletion() && !(_node instanceof FIDPlaylist && ((FIDPlaylist)_node).isSoup()) && myPredicate.evaluate(_node);
    return qualifies;
  }
