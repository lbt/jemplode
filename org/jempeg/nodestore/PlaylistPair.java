/* PlaylistPair - Decompiled by JODE
...
public class PlaylistPair
{
	...
	myFID = _node.getFID();
	myGeneration = -1;
	myGenerationStr = _node.getTags().getValue("fid_generation");
    }
	...
	boolean isStale
	    = (myGeneration != 0
	       && (node == null || node.getGeneration() != myGeneration));
	return isStale;
    }
    
    public boolean isStale(IFIDNode _node) {
	boolean isStale = (myGeneration != 0
			   && (_node == null
			       || (_node.getFID() == myFID
				   && _node.getGeneration() != myGeneration)));
	return isStale;
    }
    ...
    public static boolean containsNode(PlaylistPair[] _originalPairs,
				       IFIDNode _node) {
    ...
    public static PlaylistPair[] removeNode(PlaylistPair[] _originalPairs,
					    IFIDNode _removedNode) {
	...
	if (_originalPairs != null) {
		...
		if (_originalPairs[i].myFID == removedFID) {
			...
			for (int previousPairsNum = 0; previousPairsNum < i;
			     previousPairsNum++)
			    newPairsVec
				.addElement(_originalPairs[previousPairsNum]);
		    }
