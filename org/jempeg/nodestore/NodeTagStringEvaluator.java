/* NodeTagStringEvaluator - Decompiled by JODE
...
public class NodeTagStringEvaluator
{
	...
	if (_str == null)
		...
		for (int i = leftBracketPos; i < length; i++) {
			...
			if (leftBracketPos > -1) {
			    String tagName
				= _str.substring(leftBracketPos + 1, i);
			    int colonIndex = tagName.indexOf(':');
			    int pad = -1;
			    if (colonIndex > -1) {
				String padStr
				    = tagName.substring(colonIndex + 1);
				try {
				...
				value = TagValueRetriever.getValue(myNode,
								   tagName);
			    else
				value = (TagValueRetriever.getValue
					 (myParentPlaylist, myIndex, tagName));
			    if (value == null)
				value = "";
			    try {
				if (pad >= 0) {
					...
					= new StringBuffer();
				    for (int padNum = 0; padNum < pad;
					 padNum++)
					formatBuf.append("0");
				    DecimalFormat df
					= new DecimalFormat(formatBuf
								.toString());
				    value = (df.format
