/* AbstractDeviceSettings - Decompiled by JODE
...
public abstract class AbstractDeviceSettings
    implements Serializable, IDeviceSettings
{
	...
	public ConfigLine(String _content) {
	    ...
	    if (_content.equals("\n") || _content.length() == 0
		|| _content.charAt(0) == '#' || _content.charAt(0) == ';')
		myType = Type.COMMENT;
	    else if (_content.charAt(0) == '['
		     && _content.charAt(_content.length() - 1) == ']')
		myType = Type.SECTION;
	...
	public ConfigLine(Type _type, String _content) {
	    myType = _type;
	    myContent = _content;
	}
	...
	public String toString() {
	    return ("[ConfigLine: type = " + myType + "; content = \""
		    + myContent + "\"]");
	}
    }
	...
	StringTokenizer tokenizer
	    = new StringTokenizer(_configStr, "\n", true);
	String lastToken = "";
    ...
    public String getStringValue(String _section, String _key,
	...
	if (_section.indexOf('[') != -1)
	    throw new IllegalArgumentException
		      ("You can't have a '[' in a section name.");
	if (_key.indexOf('=') != -1)
	    throw new IllegalArgumentException
		      ("You can't have an '=' in a key name.");
	...
	if (line != null)
	    ...
	    if (sectionPos == -1) {
		ConfigLine newLine
		    = new ConfigLine(ConfigLine.Type.SECTION,
				     String.valueOf('[') + _section + ']');
		myLines.addElement(newLine);
		sectionPos = myLines.size();
	    } else
		sectionPos++;
	    ConfigLine newLine
		= new ConfigLine(ConfigLine.Type.KEY, _key + '=' + _value);
	    myLines.insertElementAt(newLine, sectionPos);
	}
	myDirty = true;
    }
    
    public boolean getBooleanValue(String _section, String _key,
				   boolean _defaultValue) {
	String value
	    = getStringValue(_section, _key, _defaultValue ? "1" : "0");
	...
	String strValue
	    = getStringValue(_section, _key, String.valueOf(_defaultValue));
    ...
    public long getLongValue(String _section, String _key,
			     long _defaultValue) {
	String strValue
	    = getStringValue(_section, _key, String.valueOf(_defaultValue));
	...
	for (int sectionPos = 0;
	     ((sectionPos = findSectionPos(sectionPos, _section)) != size
	      && sectionPos != -1);
	     sectionPos++) {
	    for (int keyPos = sectionPos + 1;
		 (keyPos != size && keyPos != -1
		  && !getLine(keyPos).getType()
			  .equals(ConfigLine.Type.SECTION));
		 keyPos++) {
		ConfigLine line = getLine(keyPos);
		if (line.getType().equals(ConfigLine.Type.KEY)
		    && line.getKey().equalsIgnoreCase(_key))
		    return keyPos;
	    }
	}
	return -1;
	...
	for (int i = _startingOffset; i < size; i++) {
	    ...
	    if (configLine.getType().equals(ConfigLine.Type.SECTION)
		&& configLine.getSection().equalsIgnoreCase(_section))
		return i;
	}
	return -1;
    }
