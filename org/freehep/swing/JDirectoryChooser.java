...
public class JDirectoryChooser extends JComponent {
   ...
   */
  public JDirectoryChooser(File _currentDirectory) {
    this();
   ...
   * @param currentDirectory The directory to which the tree is initially set
   */
  public JDirectoryChooser(String _currentDirectory) {
    this();
    if (_currentDirectory != null)
      setCurrentDirectory(new File(_currentDirectory));
  }
   ...
   */
  public JDirectoryChooser(File _currentDirectory, FileSystemView view) {
    this(view);
    setCurrentDirectory(_currentDirectory);
  }
   ...
   * @param view The FileSystemView to use
   */
  public JDirectoryChooser(String _currentDirectory, FileSystemView view) {
    this(view);
    setCurrentDirectory(new File(_currentDirectory));
  }
  ...
  public void setMultiSelectionEnabled(boolean enable) {
    ...
    tree.getSelectionModel().setSelectionMode(enable ? TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION : TreeSelectionModel.SINGLE_TREE_SELECTION);
  }
  ...
  public int showDialog(Component parent) {
    ...
    DialogUtils.centerWindow(dialog);
    dialog.show();
  ...
  private JButton approve = new JButton(ResourceBundleUtils.getUIString("select"));
  private JButton cancel = new JButton(ResourceBundleUtils.getUIString("cancel"));
  ...
  private class FileRenderer extends DefaultTreeCellRenderer {
    public Component getTreeCellRendererComponent(JTree _tree, Object node, boolean _selected, boolean expanded, boolean leaf, int row, boolean _hasFocus) {
      Component comp = super.getTreeCellRendererComponent(_tree, node, _selected, expanded, leaf, row, _hasFocus);
  ...
  private class FileTreeModel implements TreeModel {
    ...
    private File[] children(Object node) {
      ...
      try {
        ...
        if (!isRoot) {
          boolean dirOnly = foo.getFileSelectionMode() == JFileChooser.DIRECTORIES_ONLY;
        ...
        QuickSort qs = new QuickSort(new CollationKeyCache(Collator.getInstance()));
        qs.sort(children);
  ...
  public static void main(String argv[]) {
    ...
    JButton b = new JButton("Show Browser") {
      public void fireActionPerformed(ActionEvent e) {
        ...
        dlg.setCurrentDirectory(new File("M:\\Tagged"));
        dlg.showDialog(this);
