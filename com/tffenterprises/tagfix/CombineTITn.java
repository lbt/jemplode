/* CombineTITn - Decompiled by JODE
...
public class CombineTITn implements FileAction
{
	...
	if (sub_title.trim().length() != 0)
	    sub_title = ", " + sub_title;
	if (title.startsWith(super_title) && title.endsWith(sub_title))
	    return false;
	String full_title = super_title + title + sub_title;
	tag.setTitle(full_title);
