...
class Mdct
{
	...
	log2n = (int) Math.rint(Math.log((double) n) / Math.log(2.0));
	this.n = n;
	int AE = 0;
	int AO = 1;
	int BE = AE + n / 2;
	...
	int CO = CE + 1;
	for (int i = 0; i < n / 4; i++) {
	    trig[AE + i * 2] = (float) Math.cos(3.141592653589793 / (double) n
						* (double) (4 * i));
	    trig[AO + i * 2] = (float) -Math.sin(3.141592653589793 / (double) n
						 * (double) (4 * i));
	    trig[BE + i * 2]
		= (float) Math.cos(3.141592653589793 / (double) (2 * n)
				   * (double) (2 * i + 1));
	    trig[BO + i * 2]
		= (float) Math.sin(3.141592653589793 / (double) (2 * n)
				   * (double) (2 * i + 1));
	}
	for (int i = 0; i < n / 8; i++) {
	    trig[CE + i * 2] = (float) Math.cos(3.141592653589793 / (double) n
						* (double) (4 * i + 2));
	    trig[CO + i * 2] = (float) -Math.sin(3.141592653589793 / (double) n
						 * (double) (4 * i + 2));
	}
	...
	for (int i = 0; i < n / 8; i++) {
	    ...
	    bitrev[i * 2] = (acc ^ 0xffffffff) & mask;
	    bitrev[i * 2 + 1] = acc;
	}
	scale = 4.0F / (float) n;
    }
