...
public class ErrorDialog extends JPanel {
	...
	protected ErrorDialog(Throwable _exception) {
		this(ExceptionUtils.getParagraph(_exception), ExceptionUtils.getChainedStackTrace(_exception));
	}
	...
	protected ErrorDialog(String _paragraph, String _details) {
		...
		add(paragraphTA, BorderLayout.NORTH);
	...
	protected void detailsVisiblityChanged(boolean _pack) {
		if (myDetailsScrollPane != null) {
			if (myDetailsVisible) {
				add(myDetailsScrollPane, BorderLayout.CENTER);
				myDetailsButton.setText(ResourceBundleUtils.getUIString("error.hideDetails.text"));
			}
			else {
				remove(myDetailsScrollPane);
				myDetailsButton.setText(ResourceBundleUtils.getUIString("error.showDetails.text"));
			}
		}
		if (_pack) { 
	...
	public static void showErrorDialog(Window _parentFrame, String _message, boolean _modal) {
		ErrorDialog errorPanel = new ErrorDialog(_message, null);
		JOptionPane optionPane = new JOptionPane(errorPanel, JOptionPane.ERROR_MESSAGE);
		JDialog dialog = optionPane.createDialog(_parentFrame, ResourceBundleUtils.getUIString("error.frameTitle"));
		dialog.setModal(_modal);
		dialog.pack();
		dialog.show();
	}

	public static void showErrorDialog(Window _parentFrame, Throwable _throwable, boolean _modal) {
