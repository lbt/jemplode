/* CompositeProgressListener - Decompiled by JODE
...
public class CompositeProgressListener implements IProgressListener
{
    ...
    public CompositeProgressListener(IProgressListener _progressListener1,
				     IProgressListener _progressListener2) {
	myProgressListener1
	    = (_progressListener1 == null
	       ? (IProgressListener) new SilentProgressListener()
	       : _progressListener1);
	myProgressListener2
	    = (_progressListener2 == null
	       ? (IProgressListener) new SilentProgressListener()
	       : _progressListener2);
    }
    
    public boolean isInteractive() {
	if (!myProgressListener1.isInteractive()
	    && !myProgressListener2.isInteractive())
	    return false;
	return true;
    }
    
    public boolean isStopRequested() {
	if (!myProgressListener1.isStopRequested()
	    && !myProgressListener2.isStopRequested())
	    return false;
	return true;
    }
	...
	if (!myProgressListener1.isInProgress()
	    && !myProgressListener2.isInProgress())
	    return false;
	return true;
    }
