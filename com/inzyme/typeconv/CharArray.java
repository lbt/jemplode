...
public class CharArray implements IPrimitive
{
	...
	byte[] bytez;
	try {
	    bytez = _value.getBytes("ISO-8859-1");
	...
	int actualLength
	    = StringUtils.getNullTerminatedLength(myValue, _offsetFromEnd);
	String strValue;
	try {
	    if (_encoding == "default")
    ...
    public void read(int _length, LittleEndianInputStream _inputStream)
	throws IOException {
	setLength(_length);
	read(_inputStream);
    }
